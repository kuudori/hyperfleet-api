...
func conditionSubfieldConverter(n *tsl.Node, conditionType, subfield string) (interface{}, *errors.ServiceError) {
	...
	if conditionTimeSubfields[subfield] {
		...
		case tsl.KindTimestampLiteral:
			...
			rightStr = t.Format(time.RFC3339)
		default:
			return nil, errors.BadRequest(
				"expected string timestamp value for condition subfield '%s'", subfield,
			)
...
// NormalizeInSyntax converts old-style IN (...) to v6-style IN [...] in unquoted context.
// v6 TSL changed array syntax from parentheses to brackets.
func NormalizeInSyntax(search string) string {
	if !strings.Contains(strings.ToLower(search), "in") {
		return search
	}
	b := []byte(search)
	n := len(b)
	for i := 0; i < n; i++ {
		...
		if (b[i]|0x20) != 'i' || i+1 >= n || (b[i+1]|0x20) != 'n' {
			continue
		}
		if i > 0 && b[i-1] != ' ' && b[i-1] != '\t' && b[i-1] != '(' {
			continue
		}
		j := i + 2
		for j < n && b[j] == ' ' {
