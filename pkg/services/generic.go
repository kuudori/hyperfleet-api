...
func (s *sqlGenericService) buildSearchValues(
	...
	// Normalize old-style IN (...) to v6-style IN [...] for backward compatibility
	normalizedSearch := db.NormalizeInSyntax(listCtx.args.Search)

	// create the TSL tree — v6 handles deep identifiers natively, no preprocessing needed
	tslTreeWrapper, err := tsl.ParseTSL(normalizedSearch)
	if err != nil {
